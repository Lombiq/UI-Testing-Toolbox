...
This component helps identify the N+1 and other database query problems in your application.
...
This part of the infrastructure injects a mocked implementation that replaces the original persistence services (`IStore` and `ISession`), making operations observable.

### Entry points where the counter operates

- **Executing SQL commands:** The counter tracks storage and retrieval operations performed by the persistence subsystem on the configured database. In short, every CRUD operation is counted. The implementation supports counting the same command both with and without comparing the parameter set. You can use this to detect N+1 queries.
  - _Example:_ The command `SELECT * FROM SampleTable WHERE Id=@d` is counted each time the command text is executed and separately for each unique `Id`.
- **Reading a row from a result set:** Each row read from a result set is also counted. You can use this to detect unexpectedly large result sets that may indicate missing filtering and that can cause performance issues.
