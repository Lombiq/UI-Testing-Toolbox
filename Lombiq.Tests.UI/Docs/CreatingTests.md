...
Running monkey tests additionally can also be useful. Use `context.TestCurrentPageAsMonkeyRecursively()` to run monkey testing process, which walk through site pages and do random interactions with pages, like click, scrolling, form filling, etc. It's recommended to have 3 monkey tests that execute with different user states: as an admin, as a regular registered user and as an anonymous user. The admin test can start execution on admin dashboard page, while other tests can start on home page.
