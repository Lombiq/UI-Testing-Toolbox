    ...
    private static string GetExceptionMessage(string path, bool isFinalTry) =>
        isFinalTry
            ? $"Baseline image file not found, thus it was created automatically under the path {path}. Please set " +
              ...
              "package) UI testing assembly. If you run the test again, this newly created verification file will be " +
              "asserted against and the assertion will pass (unless the display of the app changed in the meantime)."
