...
public static class MonkeyTestingUITestContextExtensions
{
    ...
    /// <param name="startingRelativeUrl">The relative URL to start monkey testing from. Defaults to <c>"/"</c>.</param>
    ...
    /// <summary>
    /// Tests the current page as monkey. Test finishes by timeout or when the current page is left during testing. Can
    /// optionally take <paramref name="randomSeed"/> value to reproduce a test with the same randomization.
    /// </summary>
    /// <param name="options">The <see cref="MonkeyTestingOptions"/> instance to configure monkey testing.</param>
    ...
    /// <summary>
    /// Tests the current page as monkey recursively. When the current page is left during test, continues to test the
    /// other page and so on. Each page is tested for <see cref="MonkeyTestingOptions.PageTestTime"/> value of <paramref
    /// name="options"/>.
    /// </summary>
    /// <param name="options">The <see cref="MonkeyTestingOptions"/> instance to configure monkey testing.</param>
    ...
    /// <summary>
    /// Tests the frontend (i.e. pages NOT under /admin) as monkey as an authenticated user and as an anonymous user.
    /// </summary>
    /// <param name="options">The <see cref="MonkeyTestingOptions"/> instance to configure monkey testing.</param>
    /// <param name="signInDirectlyWithUserName">
    /// The username to sign in with directly. Defaults to <see cref="DefaultUser.UserName"/>.
